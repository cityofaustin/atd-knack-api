...
def main(src_app_id, dest_app_id, account):
    ...
    payload = RecordMap(app_name_src, app_name_dest, account, direction="to_finance_system", type_="user_account")
    # todo: update method
    res = post_record(payload.payload, auth_dest_app, "object_3", "create") #todo: move to config

    data_tracker_account_id = account.get("id")

    finance_account_id = res.get("id")

    data_tracker_payload = {"id" : data_tracker_account_id, "field_3446" : finance_account_id} #todo: move to config

    # update data tracker wifn account id in finance system
    res = post_record(data_tracker_payload, KNACK_CREDENTIALS[src_app_id], "object_9", "update") #todo: move to conifg
